...
with tab3:
    ...
    df_long = df_long.dropna(subset=["Scores"])
    df_long = df_long[df_long["Scores"] > 0]
...
# =========================================================
# AI INSIGHTS (FINAL – ROBUST + BUG-FREE)
# =========================================================
with tab5:
    ...
    COL_TOTAL = df_aat.columns[29]  # Final marks

    # Ensure Marks column exists safely
    df_aat["TotalMarks_AI"] = pd.to_numeric(df_aat[COL_TOTAL], errors="coerce")
    ...
    # =====================================================
    # BUILD DISCIPLINE METRICS
    # =====================================================
    for disc in sorted(df_fac[FAC_DISC].dropna().unique()):

        try:
            fac_disc = df_fac[df_fac[FAC_DISC] == disc]
            aat_disc = df_aat[df_aat[AAT_DISC] == disc]

            total = fac_disc[FAC_UID].nunique()

            completed = aat_disc[aat_disc["Status"] == "Completed"][AAT_UID].nunique()
            partial = aat_disc[aat_disc["Status"] == "Partial"][AAT_UID].nunique()

            participated = completed + partial

            participation_rate = (participated / total * 100) if total else 0
            completion_rate = (completed / total * 100) if total else 0

            avg_marks = aat_disc["TotalMarks_AI"].mean()

            # =====================================================
            # COVERAGE (SAFE MELT)
            # =====================================================
            df_long = aat_disc.melt(
                id_vars=[AAT_UID],
                value_vars=cat_cols,
                var_name="Category_AI",
                value_name="Score_AI"
            )

            df_long["Score_AI"] = pd.to_numeric(df_long["Score_AI"], errors="coerce")

            df_long = df_long.dropna(subset=["Score_AI"])
            df_long = df_long[df_long["Score_AI"] > 0]

            if len(cat_cols) > 0:
                coverage = len(df_long["Category_AI"].unique()) / len(cat_cols) * 100
            else:
                coverage = 0
            ...
            score = (
                ...
                0.3 * completion_rate +
                0.3 * (avg_marks if pd.notna(avg_marks) else 0) +
                0.1 * coverage
            )

            results.append({
                "Discipline": disc,
                "Participation %": round(participation_rate, 2),
                "Completion %": round(completion_rate, 2),
                "Avg Marks": round(avg_marks, 2) if pd.notna(avg_marks) else 0,
                "Coverage %": round(coverage, 2),
                "Score": round(score, 2)
            })

        except Exception as e:
            # Fail-safe: skip discipline if any issue
            continue
    ...
    # =====================================================
    # 🏆 TOP & BOTTOM DISCIPLINES
    # =====================================================
    st.markdown("### 🏆 Discipline Rankings")

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### 🥇 Top 5 Disciplines")
        st.dataframe(df_rank.head(5), use_container_width=True, hide_index=True)

    with col2:
        st.markdown("#### ⚠ Bottom 5 Disciplines")
        st.dataframe(df_rank.tail(5), use_container_width=True, hide_index=True)

    # =====================================================
    # 📊 SCORE CHART
    # =====================================================
    st.markdown("### 📊 Discipline Score Comparison")
    ...
    top_disc = df_rank.iloc[0]
    bottom_disc = df_rank.iloc[-1]

    overall_participation = df_rank["Participation %"].mean()
    overall_completion = df_rank["Completion %"].mean()

    st.markdown(f"""
### 📌 Institutional Performance Summary

- Average participation across disciplines is **{overall_participation:.2f}%**
- Average completion stands at **{overall_completion:.2f}%**

### 🏆 Top Performing Discipline
- **{top_disc['Discipline']}**
- Composite score: **{top_disc['Score']}**

### ⚠ Discipline Needing Attention
- **{bottom_disc['Discipline']}**
- Lower engagement and/or performance indicators

### 🎯 Key Observations
- Uneven participation across disciplines
- Presence of partial completions indicating drop-offs
- Coverage gaps suggest limited exposure to some categories

### 🚀 Recommendations
- Focus on bottom-ranked disciplines
- Convert partial → completed participants
- Conduct targeted training for uncovered categories
- Share best practices from top-performing disciplines
""")
